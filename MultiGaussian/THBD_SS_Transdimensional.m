function [LocSave, AmpSave, WidSave, SigSave, NC1Save, NC2Save, LikSave, prior] = ...
    THBD_SS_Transdimensional(time, P, D, ManArgs, OptArgs)
...
LocSave = zeros(totalSavedSteps, 2); % These are 2-element vectors %
AmpSave = zeros(totalSavedSteps, 2); %   since transdimensional    %
WidSave = zeros(totalSavedSteps, 2); %     (max 2 Gaussians)       %
...
% model = create_initial_model_transdimensional(prior);
model = create_initial_model_TD_customize(prior, [5 40], [-0.3 0.02], [1.0 3.0]); %%%%%%%%%%%%%% debug
model.LikeProb = calculate_like_prob_transdimensional(P, D, model, prior, 1);
...
for iStep = 1:totalSteps
    ...
    for iAction = 1:totalActions
        ...
        model_new = create_new_model_transdimensional(model, action, prior);
    ...
    if changedCorr || iStep == 1
        [model_new.LikeProb, D_model, R_LT ,R_UT ,R_P, LogDetR] = ...
            calculate_like_prob_transdimensional(P, D, model_new, prior, 1);
        ...
        [model_new.LikeProb, D_model, ~, ~, ~, ~] = ...
            calculate_like_prob_transdimensional(P, D, model_new, prior, 0, R_LT ,R_UT ,R_P, LogDetR);
    ...
    if mod((iStep - 1), saveSteps) == 0
        ...
        % ylim([LikSave(max(1, iSaveStep - 100)) LikSave(iSaveStep)+1]);
